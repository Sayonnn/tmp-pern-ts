...
class AdminService {
	...
	public auth = {
		...
		/** 2fa */
		get2FAProof:() => 
			fetchData({url: `${AdminService.apiUrl}/get-2fa-proof`}),

		set2FAProof:() => 
			postDatas({url: `${AdminService.apiUrl}/set-2fa-proof`}),
		
		twoFASetup: (data: { username: string, role: string }) =>
			postDatas({url: `${AdminService.apiUrl}/${AdminService.appName}-admin/2fa/setup`,data}),

		twoFAVerify: (data: { token: string; secret: string; username: string, role: string }) =>
			postDatas({url: `${AdminService.apiUrl}/${AdminService.appName}-admin/2fa/verify`,data}),
